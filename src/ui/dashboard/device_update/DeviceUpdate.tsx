...
export const DashboardDeviceUpdatePage: FC = () => {
    ...
    useEffect(() => {
        if (!!fetchDeviceState && !fetchDeviceState.loading) {
            if (fetchDeviceState.hasError) {
                notification.error({
                    message:
                        fetchDeviceState.error?.message || "Error al obtener el dispositivo.",
                });
            }
            onFetchDeviceStateReceived();
        }
    ...
    useEffect(() => {
        if (!!fetchUnitListState && !fetchUnitListState.loading) {
            if (fetchUnitListState.hasError) {
                notification.error({
                    message:
                        fetchUnitListState.error?.message || "Error al obtener las unidades.",
                });
            }
    ...
    useEffect(() => {
        if (!!updateDeviceState && !updateDeviceState.loading) {
            if (updateDeviceState.hasError) {
                notification.error({
                    message:
                        updateDeviceState.error?.message || "Error al actualizar el dispositivo.",
                });
            } else {
    ...
    return (
        ...
        >
            ...
            />
            <DashboardSubpageHeader title={"Dispositivo"} subtitle={device?.description}/>
            <div
                className={
            ...
            >
                <div className={"max-w-full w-full p-2"}>
                    {device ? (
                        ...
                        <EmptyData description={"Dispositivo no encontrado"} title={"Error"}/>
                    )}
                </div>
            </div>
            {reasonVisible && (
