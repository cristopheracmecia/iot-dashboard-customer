...
export const DashboardCustomerListPage: FC = () => {
    const {
        fetchList, customerList, fetchListState, onFetchListStateReceived
    ...
    const onItemClicked = useCallback((item: Customer) => {
    ...
    return <DashboardSubpageContainer className={"w-full h-full overflow-hidden"}>
        <DashboardSubpageHeader title={"Clientes"} subtitle={`${customerList?.length} clientes`} extra={<Button.Group>
            <Button type={"primary"} icon={<FontAwesomeIcon icon={faRefresh}/>} onClick={fetchList}>Actualizar</Button>
            <Button type={"dashed"} icon={<FontAwesomeIcon icon={faPlus}/>} onClick={onNewCustomerClick}>Nuevo</Button>
        </Button.Group>}/>
