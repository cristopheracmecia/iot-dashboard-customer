...
export const apiService = {
  ...
  getWithAuth: async (url: string, config?: AxiosRequestConfig) =>
  ...
  put: async (url: string, config?: AxiosRequestConfig) =>
    await RemoteApiClient.put(url, config),
  putWithAuth: async (url: string, config?: AxiosRequestConfig) =>
    await RemoteApiClient.put(url, {
      ...config,
      headers: {
        Authorization: getAccessToken(),
        "X-RefreshToken": getRefreshToken(),
